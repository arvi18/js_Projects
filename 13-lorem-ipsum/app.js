...
$(".btn-generate").click((e)=>{
  const inpValu= parseInt($("#amount").val());
  //stop submitting form
  e.preventDefault();
  if(isNaN(inpValu) || inpValu<0 || inpValu>9){
    console.log('inpValu:', inpValu)
    alert("Please input value between 1 and 9!")
  }
  if(true){
    for(let i=0;i<inpValu;i++){
      outputLorem(outRand())
    }
  }
});

function outputLorem(val){
  $(".section-center").append(`<p>${ val }</p>`)
}
function outRand(){
  rand= Math.round((Math.random(1)*(text.length-1)))
  return text[rand]
}